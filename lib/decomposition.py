...
class PCA:
  ...
  def _calculateEigenvalues(self, S):
    minSize = min(self.inputDF.shape)
    maxSize = max(self.inputDF.shape)
    eigenvalues = S ** 2 / (maxSize - 1)
    pcStrings = ['PC{}'.format(x) for x in range(1, minSize + 1)]
  ...
  def _calculateLoadings(self, V):
    ...
    self.__loadings_.index = ['PC{}'.format(x) for x in range(1, min(self.inputDF.shape)+1)]
